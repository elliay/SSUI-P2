...
export class Row extends Group {
    ...
    protected override _doLocalSizing(): void {
        ...
        for (let child of this.children) {
            // Sum up the min, natural, and max widths
            minWidth += child.minW;
            naturalWidth += child.naturalW;
            maxWidth += child.maxW;

            // Track the max height across children
            minHeight = Math.max(minHeight, child.minH);
            naturalHeight = Math.max(naturalHeight, child.naturalH);
            maxHeight = Math.max(maxHeight, child.maxH);
        }
    ...
    protected _expandChildSprings(excess: number, numSprings: number): void {
        //=== YOUR CODE HERE ===
        if (numSprings === 0) return; // If no springs, exit
