...
export class Column extends Group {
    ...
    protected override _doLocalSizing(): void {
        ...
        for (let child of this.children) {
            // Sum up the min, natural, and max heights
            minHeight += child.minH;
            naturalHeight += child.naturalH;
            maxHeight += child.maxH;

            // Track the max width across children
            minWidth = Math.max(minWidth, child.minW);
